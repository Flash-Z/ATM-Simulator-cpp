...
{  
    ui.insert_card.welcome();

    string card_number, password;
    ui.insert_card.input_card_number();
               ...
               user_ID_ >> password_ >> balance_ >> input_error_times_ >> daily_withdraw_ >> is_locked_)
        {
            ...
            if (card_number == account_number_)
            {
                if (is_locked_ == 1)
                {
                    file_line_ = 0;
                    user_file.close();
                    ui.insert_card.card_locked();
                    ui.back();
                    return false;
                }
                while (true)
                {
                    ui.insert_card.input_password();
                    cin >> password;
                    if (password == "0")
                    {
                        user_file.close();
                    ...
                    else
                    {
                        ...
                        save(file_line_);
                        ui.insert_card.error_password(3 - input_error_times_);
                    }
                }
            }
        }
